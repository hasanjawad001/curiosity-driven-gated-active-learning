if __name__=='__main__':
    ...
    ## changeR
    plt.hist(r2_score.flatten())
    
    ## changeR
    
    import numpy as np
    percentiles = np.arange(0.02, 1.0, 0.2)
    values = np.percentile(r2_score.flatten(), percentiles * 100)
    total = r2_score.flatten().shape[0]
    for p, v in zip(percentiles, values):
      above = np.sum(r2_score.flatten() >= v)
      print(f"  {p:.2f} → {v:.4f}  |  above: {above}/{total} ({100*above/total:.1f}%)")
    ...
    for trial_no in range(n_trials):
        ...
        X, y = norm_0to1(images), norm_0to1_axis1(spectra)
        ...
        ##
        ## 3a
        dim_in = X[0].shape  # Input dimensions (image height and width)
        dim_out = len(y[0])  # Output dimensions (spectra length)
        ...
        beta1=0.05
        ##
        nb_filters_enc = 32 ## 32
        nb_filters_dec = 32 ## 32
        dropout = 0.3 ## 0.3
        conv1_channels = 16
        conv2_channels = 32
        ##
        ...
        idx = 956
        patch = X[idx]  # shape: (16, 16)
        coord_y, coord_x = coordinates[idx]  # (y, x)
        print(coordinates[idx])
        
        # Patch size and half-size for centering
        patch_size = 16
        half = patch_size // 2
        ...
        # Left: full image with patch marker + bounding box
        ax1 = plt.subplot(1, 2, 1)
        ax1.imshow(full_image, origin='lower', cmap='viridis')
        ax1.scatter(coord_x, coord_y, c='white', s=100, marker='X')
        
        # Draw rectangle around the patch
        rect = patches.Rectangle(
            (coord_x - half, coord_y - half),  # bottom-left corner (x, y)
            patch_size, patch_size,            # width, height
            linewidth=1.5, edgecolor='red', facecolor='none'
        )
        ax1.add_patch(rect)
        ax1.set_title("Patch Location on Full Image")
        # ax1.legend()
        
        # Right: extracted patch
        ax2 = plt.subplot(1, 2, 2)
        ax2.imshow(patch, cmap='viridis', origin='lower')
        ax2.set_title("Extracted Patch")
        
        plt.tight_layout()
        plt.savefig(f"outputs/plots/{exp_no}_patch_no_{idx}.png", dpi=300)
        plt.show()
        ...
        def get_noisy_spectra(coordinates, y, noisy_map, r2_score):
            has_seen = False
            coords = np.array(coordinates)
            yN = y.copy()
            noise_level = np.zeros(len(y))
            r2_level = np.zeros(len(y))    
            noisy_indices = []
            ...
            return coords, yN, noise_level, noisy_indices, r2_level
            
        coords, yN, noise_level, noisy_indices, r2_level = get_noisy_spectra(coordinates, y, noisy_map, r2_score)
        
        print(coords.shape, y.shape, yN.shape, noise_level.shape, noisy_indices.shape, r2_level.shape)
        ...
        for i in range(j, j+1):  # Plot first 10 spectra
            plt.plot(y[i], label=f"R2-score: {r2_level[j]:.2f}", alpha=0.7, color='b')    
        plt.title("")
        ...
        j = 1010 # 1220
        for i in range(j, j+1):  # Plot first 10 spectra 
            plt.plot(yN[i], label=f"R2-score: {r2_level[j]:.2f}", alpha=0.7, color= 'r')
        plt.title("")
        plt.xlabel("Dimension")
        plt.ylabel("Intensity")
        plt.legend()
        ...
        plt.figure(figsize=(5, 3))
        j = 956
        plt.plot(v_step, y[j], label=f"R2-score: {r2_level[j]:.2f}", alpha=0.7, color='b')
        ...
        plt.figure(figsize=(5, 3))
        j = 1010
        plt.plot(v_step, yN[j], label=f"R2-score: {r2_level[j]:.2f}", alpha=0.7, color='r')
        plt.title(f"Sample {j}: Clean vs Noisy")
        plt.xlabel("Voltage (V)")
        plt.ylabel("Intensity")
        plt.legend()
        ...
        yN_train, yN_val, yN_test = yN[indices_train], yN[indices_val], yN[indices_test]
        ...
        try:
            plot_spectra_distribution(y_train, y_val, y_test, exp_no=exp_no, latent_dim=latent_dim, alpha=alpha, beta=beta, gamma=gamma, r_main=r_main, step=-1)
        except Exception as e:
            log_message(f'Error ({exp_no}, {latent_dim}, {alpha}, {beta}, {gamma}, {r_main}, {i}): {e}')
        print(y_train.shape, y_val.shape, y_test.shape)
        print(yN_train.shape, yN_val.shape, yN_test.shape)
        ...
        # ##
        # plt.tight_layout()
        # plt.savefig(f"outputs/plots/{exp_no}_noise_map_GP_-1c.png", dpi=300)
        # plt.show()
        ...
        # plt.tight_layout()
        # plt.savefig(f"outputs/plots/{exp_no}_noise_map_GP_-1d.png", dpi=300)
        # plt.show()
        ...
        train_dataset_init = SpectraDataset(X_train, norm_0to1_axis1(yN_train))
        val_dataset_init = SpectraDataset(X_val, norm_0to1_axis1(yN_val))
        im2spec_init = im2spec(feature_size=dim_in, target_size=dim_out, latent_dim=latent_dim, nb_filters_enc = nb_filters_enc, nb_filters_dec = nb_filters_dec, dropout = dropout).to(device)
        im2multi_init = im2multi(feature_size=dim_in, target_size=dim_out, latent_dim=latent_dim, nb_filters_enc = nb_filters_enc, nb_filters_dec = nb_filters_dec, dropout = dropout).to(device)
        # im2spec_init = train_im2spec(im2spec_init, train_dataset_init, val_dataset_init, device, num_epochs=init_num_epochs, lr=init_lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
        
        im2multi_init.enc_conv.load_state_dict(im2spec_init.enc_conv.state_dict())
        im2multi_init.enc_fc.load_state_dict(im2spec_init.enc_fc.state_dict())
        ##
        im2multi_init.spec_fc.load_state_dict(im2spec_init.dec_fc.state_dict())
        im2multi_init.spec_atrous.load_state_dict(im2spec_init.dec_atrous.state_dict())
        im2multi_init.spec_conv.load_state_dict(im2spec_init.dec_conv.state_dict())
        im2multi_init.spec_out.load_state_dict(im2spec_init.dec_out.state_dict())
        
        error_model_init = ErrorModel(latent_dim, conv1_channels, conv2_channels).to(device)
        
        ##
        X_train_active, y_train_active = X_train.copy(), yN_train.copy()
        X_train_random, y_train_random = X_train.copy(), yN_train.copy()
        X_train_active2A, y_train_active2A = X_train.copy(), yN_train.copy()
        X_train_active2B, y_train_active2B = X_train.copy(), yN_train.copy()
        X_train_active2C, y_train_active2C = X_train.copy(), yN_train.copy()
        X_train_active2D, y_train_active2D = X_train.copy(), yN_train.copy()
        X_train_active2E, y_train_active2E = X_train.copy(), yN_train.copy()    
        X_train_active3, y_train_active3 = X_train.copy(), yN_train.copy()    
        ##
        X_val_active, y_val_active = X_val.copy(), yN_val.copy()
        X_val_random, y_val_random = X_val.copy(), yN_val.copy()
        X_val_active2A, y_val_active2A = X_val.copy(), yN_val.copy()
        X_val_active2B, y_val_active2B = X_val.copy(), yN_val.copy()
        X_val_active2C, y_val_active2C = X_val.copy(), yN_val.copy()
        X_val_active2D, y_val_active2D = X_val.copy(), yN_val.copy()
        X_val_active2E, y_val_active2E = X_val.copy(), yN_val.copy()    
        X_val_active3, y_val_active3 = X_val.copy(), yN_val.copy()
        ##
        X_test_active, y_test_active, yN_test_active  = X_test.copy(), y_test.copy(), yN_test.copy()
        X_test_random, y_test_random, yN_test_random = X_test.copy(), y_test.copy(), yN_test.copy()
        X_test_active2A, y_test_active2A, yN_test_active2A = X_test.copy(), y_test.copy(), yN_test.copy()
        X_test_active2B, y_test_active2B, yN_test_active2B = X_test.copy(), y_test.copy(), yN_test.copy()
        X_test_active2C, y_test_active2C, yN_test_active2C = X_test.copy(), y_test.copy(), yN_test.copy()
        X_test_active2D, y_test_active2D, yN_test_active2D = X_test.copy(), y_test.copy(), yN_test.copy()
        X_test_active2E, y_test_active2E, yN_test_active2E = X_test.copy(), y_test.copy(), yN_test.copy()    
        X_test_active3, y_test_active3, yN_test_active3 = X_test.copy(), y_test.copy(), yN_test.copy()
        ##
        ...
        ##
        indices_train_active2A_gp = indices_train.copy() ## indices_train_val.copy()
        indices_train_active2B_gp = indices_train.copy() ## indices_train_val.copy()
        indices_train_active2C_gp = indices_train.copy() ## indices_train_val.copy()
        indices_train_active2D_gp = indices_train.copy() ## indices_train_val.copy()
        indices_train_active2E_gp = indices_train.copy() ## indices_train_val.copy()
        ...
        for i in range(exploration_steps):
            if terminate_check:
                # r2_threshold = min(0.8, r2_threshold + 0.1)
            ...
            ## train model
            if i==0:
                im2spec_active = copy.deepcopy(im2spec_init)
                ##
                ...
                # errors_test_active = compute_mse(im2spec_active, X_test_active, y_test_active, device)
                errors_test_active = compute_mse(im2spec_active, X_test_active_clean, norm_0to1_axis1(y_test_active_clean), device)
                ##        
                log_error_active.append(np.mean(errors_test_active))
                
            train_dataset_active = SpectraDataset(X_train_active, norm_0to1_axis1(y_train_active))
            val_dataset_active = SpectraDataset(X_val_active, norm_0to1_axis1(y_val_active))
            ## change
            im2spec_active = train_im2spec(im2spec_active, train_dataset_active, val_dataset_active, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            errors_train_active = compute_mse(im2spec_active, X_train_active, norm_0to1_axis1(y_train_active), device)    
            errors_val_active = compute_mse(im2spec_active, X_val_active, norm_0to1_axis1(y_val_active), device)        
            ...
            with torch.no_grad():
                ...
                X_test_active_tensor = torch.tensor(X_test_active, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active_latent = im2spec_active.encoder(X_train_active_tensor).cpu().numpy()        
                X_val_active_latent = im2spec_active.encoder(X_val_active_tensor).cpu().numpy()
                X_test_active_latent = im2spec_active.encoder(X_test_active_tensor).cpu().numpy()
                ##
            ...
            error_model_active = train_error_model(error_model_active, train_error_dataset_active, val_error_dataset_active, device, num_epochs=10*num_epochs, lr=0.5*lr, batch_size=batch_size, 
                                                   wdecay=wdecay1, wloss=wloss1-1, beta=beta1)
            ## learning/update 
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active = rng.choice(len(X_test_active), num_sample_exp, replace=False)
            else:
            ...
            ############################    
            X_train_active = np.vstack((X_train_active, X_test_active[indices_next_active]))
            # y_train_active = np.vstack((y_train_active, y_test_active[indices_next_active]))
            y_train_active = np.vstack((y_train_active, yN_test_active[indices_next_active]))
            ...
            yN_test_active = np.delete(yN_test_active, indices_next_active, axis=0)    
            ...
            # print()    
            log_message(f'active shape: {X_train_active.shape}, {y_train_active.shape}, {X_test_active.shape}, {y_test_active.shape}, {yN_test_active.shape}')
            ...
            ## train model
            if i == 0:
                # im2spec_random = im2spec(feature_size=dim_in, target_size=dim_out, latent_dim=latent_dim).to(device)
                im2spec_random = copy.deepcopy(im2spec_init)
                ...
                errors_test_random = compute_mse(im2spec_random, X_test_random_clean, norm_0to1_axis1(y_test_random_clean), device)
                log_error_random.append(np.mean(errors_test_random))
                
            train_dataset_random = SpectraDataset(X_train_random, norm_0to1_axis1(y_train_random))
            val_dataset_random = SpectraDataset(X_val_random, norm_0to1_axis1(y_val_random))
            ## change
            im2spec_random = train_im2spec(im2spec_random, train_dataset_random, val_dataset_random, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            mask_random_clean = ~np.isin(indices_test_random, noisy_indices)
            X_test_random_clean = X_test_random[mask_random_clean]
            y_test_random_clean = y_test_random[mask_random_clean]        
            ...
            ## learning/update
            ############################
            indices_next_random = rng.choice(len(X_test_random), num_sample_exp, replace=False)
            ############################
            X_train_random = np.vstack((X_train_random, X_test_random[indices_next_random]))
            # y_train_random = np.vstack((y_train_random, y_test_random[indices_next_random]))
            y_train_random = np.vstack((y_train_random, yN_test_random[indices_next_random]))    
            ...
            yN_test_random = np.delete(yN_test_random, indices_next_random, axis=0)    
            ...
            # print()    
            log_message(f'random shape: {X_train_random.shape}, {y_train_random.shape}, {X_test_random.shape}, {y_test_random.shape}, {yN_test_random.shape}')
        
            ## logging, plotting, checkpoints
            ## active2A
            ## train model
            if i==0:
                im2spec_active2A = copy.deepcopy(im2spec_init)
                ...
                errors_test_active2A = compute_mse(im2spec_active2A, X_test_active2A_clean, norm_0to1_axis1(y_test_active2A_clean), device)
                log_error_active2A.append(np.mean(errors_test_active2A))
                
            train_dataset_active2A = SpectraDataset(X_train_active2A, norm_0to1_axis1(y_train_active2A))
            val_dataset_active2A = SpectraDataset(X_val_active2A, norm_0to1_axis1(y_val_active2A))
            ## change
            im2spec_active2A = train_im2spec(im2spec_active2A, train_dataset_active2A, val_dataset_active2A, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            errors_train_active2A = compute_mse(im2spec_active2A, X_train_active2A, norm_0to1_axis1(y_train_active2A), device)    
            errors_val_active2A = compute_mse(im2spec_active2A, X_val_active2A, norm_0to1_axis1(y_val_active2A), device)        
            ...
            with torch.no_grad():
                ...
                X_test_active2A_tensor = torch.tensor(X_test_active2A, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active2A_latent = im2spec_active2A.encoder(X_train_active2A_tensor).cpu().numpy()        
                X_val_active2A_latent = im2spec_active2A.encoder(X_val_active2A_tensor).cpu().numpy()
                X_test_active2A_latent = im2spec_active2A.encoder(X_test_active2A_tensor).cpu().numpy()
                ##
                # Normalize using training set stats
            ...
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active2A = rng.choice(len(X_test_active2A), num_sample_exp, replace=False)
            else:
                ...
                acquisition_values_active2A = acquisition_function(predicted_errors_active2A, distances_active2A, sims_active2A, alpha=alpha, beta=beta, gamma=gamma, gp=gpA, indices=indices_test_active2A, coordinates=coordinates, r2_threshold=r2_thresholdA)
                num_sample_exp_active2A = max(len(indices_train_random) - len(indices_train_active2A), num_sample_exp)
                indices_next_active2A = np.argsort(acquisition_values_active2A)[-num_sample_exp_active2A:]
            ############################    
            X_train_active2A = np.vstack((X_train_active2A, X_test_active2A[indices_next_active2A]))
            # y_train_active2A = np.vstack((y_train_active2A, y_test_active2A[indices_next_active2A]))
            y_train_active2A = np.vstack((y_train_active2A, yN_test_active2A[indices_next_active2A]))    
            ...
            yN_test_active2A = np.delete(yN_test_active2A, indices_next_active2A, axis=0)    
            selected_indices_active2A = indices_test_active2A[indices_next_active2A]        
            # print('active2A')
            # print(len(np.intersect1d(noisy_indices, indices_train_active2A)), len(np.intersect1d(noisy_indices, selected_indices_active2A)), len(np.intersect1d(noisy_indices, indices_test_active2A)), len(np.intersect1d(noisy_indices, indices_train_active2A_gp)))                    
            ...
            # print()        
            log_message(f'active2A shape: {X_train_active2A.shape}, {y_train_active2A.shape}, {X_test_active2A.shape}, {y_test_active2A.shape}')
    
            ## logging, plotting, checkpoints
            ## active2B
            ## train model
            if i==0:
                im2spec_active2B = copy.deepcopy(im2spec_init)
                ...
                errors_test_active2B = compute_mse(im2spec_active2B, X_test_active2B_clean, norm_0to1_axis1(y_test_active2B_clean), device)
                log_error_active2B.append(np.mean(errors_test_active2B))
                
            train_dataset_active2B = SpectraDataset(X_train_active2B, norm_0to1_axis1(y_train_active2B))
            val_dataset_active2B = SpectraDataset(X_val_active2B, norm_0to1_axis1(y_val_active2B))
            ## change
            im2spec_active2B = train_im2spec(im2spec_active2B, train_dataset_active2B, val_dataset_active2B, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            errors_train_active2B = compute_mse(im2spec_active2B, X_train_active2B, norm_0to1_axis1(y_train_active2B), device)    
            errors_val_active2B = compute_mse(im2spec_active2B, X_val_active2B, norm_0to1_axis1(y_val_active2B), device)        
            ...
            with torch.no_grad():
                ...
                X_test_active2B_tensor = torch.tensor(X_test_active2B, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active2B_latent = im2spec_active2B.encoder(X_train_active2B_tensor).cpu().numpy()        
                X_val_active2B_latent = im2spec_active2B.encoder(X_val_active2B_tensor).cpu().numpy()
                X_test_active2B_latent = im2spec_active2B.encoder(X_test_active2B_tensor).cpu().numpy()
                ##
                # Normalize using training set stats
            ...
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active2B = rng.choice(len(X_test_active2B), num_sample_exp, replace=False)
            else:
                ...
                acquisition_values_active2B = acquisition_function(predicted_errors_active2B, distances_active2B, sims_active2B, alpha=alpha, beta=beta, gamma=gamma, gp=gpB, indices=indices_test_active2B, coordinates=coordinates, r2_threshold=r2_thresholdB)
                num_sample_exp_active2B = max(len(indices_train_random) - len(indices_train_active2B), num_sample_exp)
                indices_next_active2B = np.argsort(acquisition_values_active2B)[-num_sample_exp_active2B:]
            ############################    
            X_train_active2B = np.vstack((X_train_active2B, X_test_active2B[indices_next_active2B]))
            # y_train_active2B = np.vstack((y_train_active2B, y_test_active2B[indices_next_active2B]))
            y_train_active2B = np.vstack((y_train_active2B, yN_test_active2B[indices_next_active2B]))    
            ...
            yN_test_active2B = np.delete(yN_test_active2B, indices_next_active2B, axis=0)    
            selected_indices_active2B = indices_test_active2B[indices_next_active2B]        
            # print('active2B')
            # print(len(np.intersect1d(noisy_indices, indices_train_active2B)), len(np.intersect1d(noisy_indices, selected_indices_active2B)), len(np.intersect1d(noisy_indices, indices_test_active2B)), len(np.intersect1d(noisy_indices, indices_train_active2B_gp)))                    
            ...
            # print()        
            log_message(f'active2B shape: {X_train_active2B.shape}, {y_train_active2B.shape}, {X_test_active2B.shape}, {y_test_active2B.shape}')
    
            ## logging, plotting, checkpoints
            ## active2C
            ## train model
            if i==0:
                im2spec_active2C = copy.deepcopy(im2spec_init)
                ...
                errors_test_active2C = compute_mse(im2spec_active2C, X_test_active2C_clean, norm_0to1_axis1(y_test_active2C_clean), device)
                log_error_active2C.append(np.mean(errors_test_active2C))
                
            train_dataset_active2C = SpectraDataset(X_train_active2C, norm_0to1_axis1(y_train_active2C))
            val_dataset_active2C = SpectraDataset(X_val_active2C, norm_0to1_axis1(y_val_active2C))
            ## change
            im2spec_active2C = train_im2spec(im2spec_active2C, train_dataset_active2C, val_dataset_active2C, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            errors_train_active2C = compute_mse(im2spec_active2C, X_train_active2C, norm_0to1_axis1(y_train_active2C), device)    
            errors_val_active2C = compute_mse(im2spec_active2C, X_val_active2C, norm_0to1_axis1(y_val_active2C), device)        
            ...
            with torch.no_grad():
                ...
                X_test_active2C_tensor = torch.tensor(X_test_active2C, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active2C_latent = im2spec_active2C.encoder(X_train_active2C_tensor).cpu().numpy()        
                X_val_active2C_latent = im2spec_active2C.encoder(X_val_active2C_tensor).cpu().numpy()
                X_test_active2C_latent = im2spec_active2C.encoder(X_test_active2C_tensor).cpu().numpy()
                ##
                # Normalize using training set stats
            ...
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active2C = rng.choice(len(X_test_active2C), num_sample_exp, replace=False)
            else:
                ...
                acquisition_values_active2C = acquisition_function(predicted_errors_active2C, distances_active2C, sims_active2C, alpha=alpha, beta=beta, gamma=gamma, gp=gpC, indices=indices_test_active2C, coordinates=coordinates, r2_threshold=r2_thresholdC)
                num_sample_exp_active2C = max(len(indices_train_random) - len(indices_train_active2C), num_sample_exp)
                indices_next_active2C = np.argsort(acquisition_values_active2C)[-num_sample_exp_active2C:]
            ############################    
            X_train_active2C = np.vstack((X_train_active2C, X_test_active2C[indices_next_active2C]))
            # y_train_active2C = np.vstack((y_train_active2C, y_test_active2C[indices_next_active2C]))
            y_train_active2C = np.vstack((y_train_active2C, yN_test_active2C[indices_next_active2C]))    
            ...
            yN_test_active2C = np.delete(yN_test_active2C, indices_next_active2C, axis=0)    
            selected_indices_active2C = indices_test_active2C[indices_next_active2C]        
            # print('active2C')
            # print(len(np.intersect1d(noisy_indices, indices_train_active2C)), len(np.intersect1d(noisy_indices, selected_indices_active2C)), len(np.intersect1d(noisy_indices, indices_test_active2C)), len(np.intersect1d(noisy_indices, indices_train_active2C_gp)))                    
            ...
            # print()        
            log_message(f'active2C shape: {X_train_active2C.shape}, {y_train_active2C.shape}, {X_test_active2C.shape}, {y_test_active2C.shape}')
    
            ## logging, plotting, checkpoints
            ## active2D
            ## train model
            if i==0:
                im2spec_active2D = copy.deepcopy(im2spec_init)
                ...
                errors_test_active2D = compute_mse(im2spec_active2D, X_test_active2D_clean, norm_0to1_axis1(y_test_active2D_clean), device)
                log_error_active2D.append(np.mean(errors_test_active2D))
                
            train_dataset_active2D = SpectraDataset(X_train_active2D, norm_0to1_axis1(y_train_active2D))
            val_dataset_active2D = SpectraDataset(X_val_active2D, norm_0to1_axis1(y_val_active2D))
            ## change
            im2spec_active2D = train_im2spec(im2spec_active2D, train_dataset_active2D, val_dataset_active2D, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            errors_train_active2D = compute_mse(im2spec_active2D, X_train_active2D, norm_0to1_axis1(y_train_active2D), device)    
            errors_val_active2D = compute_mse(im2spec_active2D, X_val_active2D, norm_0to1_axis1(y_val_active2D), device)        
            ...
            with torch.no_grad():
                ...
                X_test_active2D_tensor = torch.tensor(X_test_active2D, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active2D_latent = im2spec_active2D.encoder(X_train_active2D_tensor).cpu().numpy()        
                X_val_active2D_latent = im2spec_active2D.encoder(X_val_active2D_tensor).cpu().numpy()
                X_test_active2D_latent = im2spec_active2D.encoder(X_test_active2D_tensor).cpu().numpy()
                ##
                # Normalize using training set stats
            ...
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active2D = rng.choice(len(X_test_active2D), num_sample_exp, replace=False)
            else:
                ...
                acquisition_values_active2D = acquisition_function(predicted_errors_active2D, distances_active2D, sims_active2D, alpha=alpha, beta=beta, gamma=gamma, gp=gpD, indices=indices_test_active2D, coordinates=coordinates, r2_threshold=r2_thresholdD)
                num_sample_exp_active2D = max(len(indices_train_random) - len(indices_train_active2D), num_sample_exp)
                indices_next_active2D = np.argsort(acquisition_values_active2D)[-num_sample_exp_active2D:]
            ############################    
            X_train_active2D = np.vstack((X_train_active2D, X_test_active2D[indices_next_active2D]))
            # y_train_active2D = np.vstack((y_train_active2D, y_test_active2D[indices_next_active2D]))
            y_train_active2D = np.vstack((y_train_active2D, yN_test_active2D[indices_next_active2D]))    
            ...
            yN_test_active2D = np.delete(yN_test_active2D, indices_next_active2D, axis=0)    
            selected_indices_active2D = indices_test_active2D[indices_next_active2D]        
            # print('active2D')
            # print(len(np.intersect1d(noisy_indices, indices_train_active2D)), len(np.intersect1d(noisy_indices, selected_indices_active2D)), len(np.intersect1d(noisy_indices, indices_test_active2D)), len(np.intersect1d(noisy_indices, indices_train_active2D_gp)))                    
            ...
            # print()        
            log_message(f'active2D shape: {X_train_active2D.shape}, {y_train_active2D.shape}, {X_test_active2D.shape}, {y_test_active2D.shape}')
    
            ## logging, plotting, checkpoints
            ## active2E
            ## train model
            if i==0:
                im2spec_active2E = copy.deepcopy(im2spec_init)
                ...
                errors_test_active2E = compute_mse(im2spec_active2E, X_test_active2E_clean, norm_0to1_axis1(y_test_active2E_clean), device)
                log_error_active2E.append(np.mean(errors_test_active2E))
                
            train_dataset_active2E = SpectraDataset(X_train_active2E, norm_0to1_axis1(y_train_active2E))
            val_dataset_active2E = SpectraDataset(X_val_active2E, norm_0to1_axis1(y_val_active2E))
            ## change
            im2spec_active2E = train_im2spec(im2spec_active2E, train_dataset_active2E, val_dataset_active2E, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            errors_train_active2E = compute_mse(im2spec_active2E, X_train_active2E, norm_0to1_axis1(y_train_active2E), device)    
            errors_val_active2E = compute_mse(im2spec_active2E, X_val_active2E, norm_0to1_axis1(y_val_active2E), device)        
            ...
            with torch.no_grad():
                ...
                X_test_active2E_tensor = torch.tensor(X_test_active2E, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active2E_latent = im2spec_active2E.encoder(X_train_active2E_tensor).cpu().numpy()        
                X_val_active2E_latent = im2spec_active2E.encoder(X_val_active2E_tensor).cpu().numpy()
                X_test_active2E_latent = im2spec_active2E.encoder(X_test_active2E_tensor).cpu().numpy()
                ##
                # Normalize using training set stats
            ...
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active2E = rng.choice(len(X_test_active2E), num_sample_exp, replace=False)
            else:
                ...
                acquisition_values_active2E = acquisition_function(predicted_errors_active2E, distances_active2E, sims_active2E, alpha=alpha, beta=beta, gamma=gamma, gp=gpE, indices=indices_test_active2E, coordinates=coordinates, r2_threshold=r2_thresholdE)
                num_sample_exp_active2E = max(len(indices_train_random) - len(indices_train_active2E), num_sample_exp)
                indices_next_active2E = np.argsort(acquisition_values_active2E)[-num_sample_exp_active2E:]
            ############################    
            X_train_active2E = np.vstack((X_train_active2E, X_test_active2E[indices_next_active2E]))
            # y_train_active2E = np.vstack((y_train_active2E, y_test_active2E[indices_next_active2E]))
            y_train_active2E = np.vstack((y_train_active2E, yN_test_active2E[indices_next_active2E]))    
            ...
            yN_test_active2E = np.delete(yN_test_active2E, indices_next_active2E, axis=0)    
            selected_indices_active2E = indices_test_active2E[indices_next_active2E]        
            # print('active2E')
            # print(len(np.intersect1d(noisy_indices, indices_train_active2E)), len(np.intersect1d(noisy_indices, selected_indices_active2E)), len(np.intersect1d(noisy_indices, indices_test_active2E)), len(np.intersect1d(noisy_indices, indices_train_active2E_gp)))                    
            ...
            # print()        
            log_message(f'active2E shape: {X_train_active2E.shape}, {y_train_active2E.shape}, {X_test_active2E.shape}, {y_test_active2E.shape}')
            ...
            ## train model
            if i==0:
                im2multi_active3 = copy.deepcopy(im2multi_init)
                ...
                errors_test_active3 = compute_mse(im2multi_active3, X_test_active3_clean, norm_0to1_axis1(y_test_active3_clean), device, multitask=True)
                log_error_active3.append(np.mean(errors_test_active3))
                
            train_dataset_active3 = SpectraDataset(X_train_active3, norm_0to1_axis1(y_train_active3))
            val_dataset_active3 = SpectraDataset(X_val_active3, norm_0to1_axis1(y_val_active3))
            ## change
            im2multi_active3 = train_im2multi(
                im2multi_active3, train_dataset_active3, val_dataset_active3, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1,
                r_main=r_main, validation=2
            )
            ## errors
            errors_train_active3 = compute_mse(im2multi_active3, X_train_active3, norm_0to1_axis1(y_train_active3), device, multitask=True)    
            errors_val_active3 = compute_mse(im2multi_active3, X_val_active3, norm_0to1_axis1(y_val_active3), device, multitask=True)        
            ...
            with torch.no_grad():
                ...
                X_test_active3_tensor = torch.tensor(X_test_active3, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active3_latent = im2multi_active3.encoder(X_train_active3_tensor).cpu().numpy()        
                X_val_active3_latent = im2multi_active3.encoder(X_val_active3_tensor).cpu().numpy()
                X_test_active3_latent = im2multi_active3.encoder(X_test_active3_tensor).cpu().numpy()
                ##
                # Normalize using training set stats
            ...
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active3 = rng.choice(len(X_test_active3), num_sample_exp, replace=False)
            else:
            ...
            ############################    
            X_train_active3 = np.vstack((X_train_active3, X_test_active3[indices_next_active3]))
            # y_train_active3 = np.vstack((y_train_active3, y_test_active3[indices_next_active3]))
            y_train_active3 = np.vstack((y_train_active3, yN_test_active3[indices_next_active3]))
            ...
            yN_test_active3 = np.delete(yN_test_active3, indices_next_active3, axis=0)    
            ...
            # print()    
            log_message(f'active3 shape: {X_train_active3.shape}, {y_train_active3.shape}, {X_test_active3.shape}, {y_test_active3.shape}, {yN_test_active3.shape}')
            ###
            ...
            im2spec_active2E.eval()            
            im2multi_active3.eval()    
            X_test_tensor = torch.tensor(X_test[indices], dtype=torch.float32).to(device)
            y_test_true = norm_0to1_axis1(y_test[indices])
            with torch.no_grad():
                pred_active = im2spec_active(X_test_tensor).cpu().numpy()
                pred_random = im2spec_random(X_test_tensor).cpu().numpy()
                pred_active2A = im2spec_active2A(X_test_tensor).cpu().numpy()
                pred_active2B = im2spec_active2B(X_test_tensor).cpu().numpy()
                pred_active2C = im2spec_active2C(X_test_tensor).cpu().numpy()
                pred_active2D = im2spec_active2D(X_test_tensor).cpu().numpy()
                pred_active2E = im2spec_active2E(X_test_tensor).cpu().numpy()                
                pred_active3, _ = im2multi_active3(X_test_tensor)
                pred_active3 = pred_active3.cpu().numpy()        
            fig, axs = plt.subplots(6, 1, figsize=(8, 18))
            axs = axs.flatten()
            for idx in range(num_samples):
                axs[idx].plot(y_test_true[idx], label='True', color='red', linestyle='--', linewidth=2)
                axs[idx].plot(pred_active[idx], label='Active', color='blue')
                axs[idx].plot(pred_random[idx], label='Random', color='orange')
                # axs[idx].plot(pred_active2[idx], label='ActiveQC', color='green')
                axs[idx].plot(pred_active2A[idx], label=f'ActiveQC τ={r2_thresholdA}', color='#2d8a2d')
                axs[idx].plot(pred_active2B[idx], label=f'ActiveQC τ={r2_thresholdB}', color='#4CAF50')
                axs[idx].plot(pred_active2C[idx], label=f'ActiveQC τ={r2_thresholdC}', color='#81C784')
                axs[idx].plot(pred_active2D[idx], label=f'ActiveQC τ={r2_thresholdD}', color='#A5D6A7')
                axs[idx].plot(pred_active2E[idx], label=f'ActiveQC τ={r2_thresholdE}', color='#C8E6C9')            
                axs[idx].plot(pred_active3[idx], label='ActiveMT', color='purple')        
                axs[idx].set_title(f"")
                axs[idx].set_xlabel("Spectral Channel")
            ...
            axs = axs.flatten()    
            for idx in range(num_samples):
                axs[idx].plot(v_step, y_test_true[idx], label='True', color='red', linestyle='--', linewidth=2)
                axs[idx].plot(v_step, pred_active[idx], label='Active', color='blue')
                axs[idx].plot(v_step, pred_random[idx], label='Random', color='orange')
                # axs[idx].plot(v_step, pred_active2[idx], label='ActiveQC', color='green')
                axs[idx].plot(v_step, pred_active2A[idx], label=f'ActiveQC τ={r2_thresholdA}', color='#2d8a2d')
                axs[idx].plot(v_step, pred_active2B[idx], label=f'ActiveQC τ={r2_thresholdB}', color='#4CAF50')
                axs[idx].plot(v_step, pred_active2C[idx], label=f'ActiveQC τ={r2_thresholdC}', color='#81C784')
                axs[idx].plot(v_step, pred_active2D[idx], label=f'ActiveQC τ={r2_thresholdD}', color='#A5D6A7')
                axs[idx].plot(v_step, pred_active2E[idx], label=f'ActiveQC τ={r2_thresholdE}', color='#C8E6C9')            
                axs[idx].plot(v_step, pred_active3[idx], label='ActiveMT', color='purple')        
                axs[idx].set_title(f"")
                axs[idx].set_xlabel("Voltage (V)")
                axs[idx].set_ylabel("Intensity")
                axs[idx].grid(True)
            handles, labels = axs[0].get_legend_handles_labels()
            fig.legend(handles, labels, loc='lower center', ncol=4, bbox_to_anchor=(0.5, -0.05))
            plt.tight_layout(rect=[0, 0.08, 1, 1])  # leave space for legend
            plot_path = f"outputs/plots/{exp_no}_result_predictions_voltage_{i}.png"
            plt.savefig(plot_path, dpi=300, bbox_inches='tight')
            plt.show()
            ...
            if i==0:
                ...
                yN_test_active2A = np.concatenate([yN_test_active2A, y_val_drop], axis=0)        
            ...
            yN_test_active2A = np.concatenate([yN_test_active2A, y_train_drop], axis=0)    
            ...
            if i==0:
                ...
                yN_test_active2B = np.concatenate([yN_test_active2B, y_val_drop], axis=0)        
            ...
            yN_test_active2B = np.concatenate([yN_test_active2B, y_train_drop], axis=0)    
            ...
            if i==0:
                ...
                yN_test_active2C = np.concatenate([yN_test_active2C, y_val_drop], axis=0)        
            ...
            yN_test_active2C = np.concatenate([yN_test_active2C, y_train_drop], axis=0)    
            ...
            if i==0:
                ...
                yN_test_active2D = np.concatenate([yN_test_active2D, y_val_drop], axis=0)        
            ...
            yN_test_active2D = np.concatenate([yN_test_active2D, y_train_drop], axis=0)    
            ...
            if i==0:
                ...
                yN_test_active2E = np.concatenate([yN_test_active2E, y_val_drop], axis=0)        
            ...
            yN_test_active2E = np.concatenate([yN_test_active2E, y_train_drop], axis=0)    
