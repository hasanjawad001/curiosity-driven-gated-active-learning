if __name__=='__main__':
    ...
    for trial_no in range(n_trials):
        ...
        X_train_active2, y_train_active2 = X_train.copy(), yN_train.copy()    
        ...
        X_val_active2, y_val_active2 = X_val.copy(), yN_val.copy()
        ...
        X_test_active2, y_test_active2, yN_test_active2 = X_test.copy(), y_test.copy(), yN_test.copy()
        ...
        ##
        indices_train_active2_gp = indices_train.copy() ## indices_train_val.copy()
        ...
        for i in range(exploration_steps):
            ...
            ## train model
            if i==0:
                im2spec_active2 = copy.deepcopy(im2spec_init)
                ...
                errors_test_active2 = compute_mse(im2spec_active2, X_test_active2_clean, norm_0to1_axis1(y_test_active2_clean), device)
                log_error_active2.append(np.mean(errors_test_active2))
                
            train_dataset_active2 = SpectraDataset(X_train_active2, norm_0to1_axis1(y_train_active2))
            val_dataset_active2 = SpectraDataset(X_val_active2, norm_0to1_axis1(y_val_active2))
            ## change
            im2spec_active2 = train_im2spec(im2spec_active2, train_dataset_active2, val_dataset_active2, device, num_epochs=num_epochs, lr=lr, batch_size=batch_size, wdecay=wdecay1, wloss=wloss1, beta=beta1)
            ## errors
            errors_train_active2 = compute_mse(im2spec_active2, X_train_active2, norm_0to1_axis1(y_train_active2), device)    
            errors_val_active2 = compute_mse(im2spec_active2, X_val_active2, norm_0to1_axis1(y_val_active2), device)        
            ...
            with torch.no_grad():
                ...
                X_test_active2_tensor = torch.tensor(X_test_active2, dtype=torch.float32).unsqueeze(1).to(device)            
                X_train_active2_latent = im2spec_active2.encoder(X_train_active2_tensor).cpu().numpy()        
                X_val_active2_latent = im2spec_active2.encoder(X_val_active2_tensor).cpu().numpy()
                X_test_active2_latent = im2spec_active2.encoder(X_test_active2_tensor).cpu().numpy()
                ##
                # Normalize using training set stats
            ...
            ############################
            if alpha == 0 and beta == 0 and gamma == 0:
                indices_next_active2 = rng.choice(len(X_test_active2), num_sample_exp, replace=False)
            else:
                ...
                acquisition_values_active2 = acquisition_function(predicted_errors_active2, distances_active2, sims_active2, alpha=alpha, beta=beta, gamma=gamma, gp=gp, indices=indices_test_active2, coordinates=coordinates, r2_threshold=r2_threshold)
                num_sample_exp_active2 = max(len(indices_train_random) - len(indices_train_active2), num_sample_exp)
                indices_next_active2 = np.argsort(acquisition_values_active2)[-num_sample_exp_active2:]
            ############################    
            X_train_active2 = np.vstack((X_train_active2, X_test_active2[indices_next_active2]))
            # y_train_active2 = np.vstack((y_train_active2, y_test_active2[indices_next_active2]))
            y_train_active2 = np.vstack((y_train_active2, yN_test_active2[indices_next_active2]))    
            ...
            yN_test_active2 = np.delete(yN_test_active2, indices_next_active2, axis=0)    
            ...
            # print()        
            log_message(f'active2 shape: {X_train_active2.shape}, {y_train_active2.shape}, {X_test_active2.shape}, {y_test_active2.shape}')
            ...
            with torch.no_grad():
                ...
                pred_active2 = im2spec_active2(X_test_tensor).cpu().numpy()
            ...
            if i==0:
                ...
                yN_test_active2 = np.concatenate([yN_test_active2, y_val_drop], axis=0)        
            ...
            yN_test_active2 = np.concatenate([yN_test_active2, y_train_drop], axis=0)    
